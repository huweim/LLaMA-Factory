...
class LinearFunction(torch.autograd.Function):
    ...
    def forward(
        ...
        if bias is not None:
            ctx.has_bias = True
            # bf_bias = pseudo_quantize_tensor(bias, n_bit=8, zero_point=False, q_group_size=group_size)
        else:
            ctx.has_bias = False
            bf_bias = None
        ...
        if use_quantize:
            ...
            bf_in = pseudo_quantize_tensor(input, n_bit=8, zero_point=False, q_group_size=group_size)
            # element-wise quantize for weight and bias
            bf_weight = pseudo_quantize_tensor(weight, n_bit=8, zero_point=False, q_group_size=group_size)
            ctx.save_for_backward(bf_in, bf_weight)
            # compute output
            output = f_linear(bf_in, bf_weight)
            # print(bf_in.dtype, bf_weight.dtype, output.dtype)
            # output = pseudo_quantize_tensor(output, n_bit=8, zero_point=False, q_group_size=group_size)
    ...
    def backward(ctx, grad_output):
        ...
        if use_quantize:
            grad_output = pseudo_quantize_tensor(grad_output.t(), n_bit=8, zero_point=False, q_group_size=group_size).t()
            # 对齐 qex_input 和 qex_grad_output 的类型，否则 matmul 无法计算；
            # input = input.half()
            qex_input = pseudo_quantize_tensor(input.t(), n_bit=8, zero_point=False, q_group_size=group_size).t()
            qex_grad_output = pseudo_quantize_tensor(grad_output.t(), n_bit=8, zero_point=False, q_group_size=group_size).t()

            # qex_input = pseudo_quantize_tensor(input, n_bit=8, zero_point=False, q_group_size=group_size)
            # qex_grad_output = pseudo_quantize_tensor(grad_output, n_bit=8, zero_point=False, q_group_size=group_size)
            ...
            grad_weight = pseudo_quantize_tensor(grad_weight, n_bit=8, zero_point=False, q_group_size=group_size)

                #####################################################
            # perform madtile operation for grad_input
            #####################################################
            # compute grad_input, quantize everything along output size
            qos_weight = pseudo_quantize_tensor(weight.t(), n_bit=8, zero_point=False, q_group_size=group_size).t()
            # qos_weight = qos_weight.half()

            # grad_output shape is (B, seq, out_dim)
            qos_grad_output = pseudo_quantize_tensor(grad_output, n_bit=8, zero_point=False, q_group_size=group_size)
            ...
            grad_input = pseudo_quantize_tensor(grad_input, n_bit=8, zero_point=False, q_group_size=group_size)
        ...
        if not ctx.has_bias:
            ...
            if use_quantize:
                grad_bias = pseudo_quantize_tensor(grad_bias, n_bit=8, zero_point=False, q_group_size=group_size)
